...
// vec3 rayDirection(float fieldOfView, vec2 vUv) {
//     vec2 xy = vUv - iResolution.xy / 2.0;
...
//     cloudHeight = saturateNew((p.y - CLOUD_START)/(CLOUD_END-CLOUD_START));

//     float bottom = 1.0-saturateNew(length(p.xz)/(1.25*CLOUD_EXTENT));
    
//     //Round top and bottom edges
//     bottom *= saturateNew(remap(cloudHeight, 0.25*bottom, 1.0, 1.0, 0.0)) 
//         	* saturateNew(remap(cloudHeight, 0.0, 0.175, 0.45, 1.0));

//     //Subtract coarse noise
//     bottom = saturateNew(remap(bottom, 0.5*getNoise(0.25*p, 0.05), 1.0, 0.0, 1.0));
//     //Subtract fine noise
//     bottom = saturateNew(remap(bottom, 0.15*getNoise(1.0*p, 0.2), 1.0, 0.0, 1.0));

//     float top = 1.0-saturateNew(length(p.xz)/(1.5*CLOUD_EXTENT));
    
//     //Round top and bottom edges
//     top *= saturateNew(remap(1.0-cloudHeight, 0.25*top, 1.0, 1.0, 0.0)) 
//         * saturateNew(remap(1.0-cloudHeight, 0.0, 0.175, 0.45, 1.0));
    
//     //Subtract coarse noise
//     top = saturateNew(remap(top, 0.5*getNoise(0.25*p, 0.05), 1.0, 0.0, 1.0));
//     //Subtract fine noise
//     top = saturateNew(remap(top, 0.15*getNoise(1.0*p, 0.2), 1.0, 0.0, 1.0));
...
//     vec3 rayDir = rayDirection(40.0, vUv);
...
//         float blueNoise = texture(iChannel1, vUv / 1024.0).r;
